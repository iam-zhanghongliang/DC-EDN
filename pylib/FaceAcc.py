...
########## rmse ##########
def per_image_rmse_p__(pred, ann):
    ...
    for i in range(N):
        ...
        if L == 7:
            ...
            #a = sum(a)/6
            #print(pts_gt[36:42],)
            #print(np.mean(pts_gt[36:42],axis=0))
            #b = sum(b)/6
            pupil = np.linalg.norm(np.mean(pts_gt[36:42],axis=0)-np.mean(pts_gt[42:48],axis=0))
        rmse[i] = np.sum(np.linalg.norm(pts_pred-pts_gt, axis=1))/(pupil*L)
    return rmse
